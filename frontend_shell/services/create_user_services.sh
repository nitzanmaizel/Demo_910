...
cat <<EOL > frontend/src/services/userServices.tsx
...
export function useCreateUserMutation() {
  ...
  return useMutation<IUser, Error, Partial<IUser>, { previousData: RIUsersType; tempId: string }>({
    ...
    onMutate: async (newUser) => {
      ...
      const optimisticUser: IUser = { ...newUser, idNumber: tempId } as IUser;
...
export function useUpdateUserMutation() {
  ...
  return useMutation<IUser, Error, IUser, { previousData: RIUsersType }>({
    mutationFn: async (updatedUser) => {
      return await fetchAPI<IUser>(\`\/user/\${updatedUser.idNumber}\`, {method: 'PUT', body: { updatedUser }});
    },
    onMutate: async (updatedUser) => {
      ...
      queryClient.setQueryData(['users'], {
        ...previousData,
        users: previousUsers.map((user) => (user.idNumber === updatedUser.idNumber ? updatedUser : user)),
      });
...
export function useDeleteUserMutation() {
  ...
  return useMutation<void, Error, string, { previousData: RIUsersType }>({
    ...
    onMutate: async (id) => {
      ...
      queryClient.setQueryData(['users'], {
        ...previousData,
        users: previousUsers.filter((user) => user.idNumber !== id),
      });

      return { previousData };
    },
    onError: (_error, _idNumber, context) => {
      queryClient.setQueryData(['users'], context?.previousData);
